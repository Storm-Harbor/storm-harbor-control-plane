(() => {
  ...
  function addCard(graph, node, x, y, width = 184, height = 72) {
    ...
    const element = new joint.shapes.standard.HeaderedRectangle({
      ...
      attrs: {
        ...
        headerText: { text: hasHeader ? node.groupLabel : "", fill: "#212529", fontFamily: "'Open Sans', sans-serif", fontSize: 8.5, fontWeight: 800, letterSpacing: .15 },
        bodyText: { text: node.label, fill: palette.ink, fontFamily: "'Open Sans', sans-serif", fontSize: 11.5, fontWeight: 700, lineHeight: 14.5, textWrap: { width: -22, height: -13, ellipsis: true } }
      }
    });
  ...
  function renderSequence(model, graph) {
    ...
    model.edges.forEach((edge, index) => {
      ...
      link.appendLabel({ attrs: { text: { text: edge.label, fill: palette.ink, fontFamily: "'Open Sans', sans-serif", fontSize: 11, fontWeight: 650 }, rect: { fill: "var(--joint-canvas-deep)", stroke: palette.line, strokeWidth: 1, rx: 3, ry: 3 } }, position: { distance: 0.5, offset: -13 } });
      graph.addCell(link);
    });
  }
  ...
  function renderGraph(model, graph) {
    ...
    model.edges.forEach((edge) => {
      ...
      if (edge.label) link.appendLabel({ attrs: { text: { text: edge.label, fill: palette.ink, fontFamily: "'Open Sans', sans-serif", fontSize: 10.5, fontWeight: 650 }, rect: { fill: "var(--joint-canvas-deep)", stroke: palette.line, rx: 3, ry: 3 } }, position: { distance: 0.5, offset: -11 } });
      graph.addCell(link);
      link.toBack();
    });
  }
  ...
  function renderDiagram(sourceElement, index) {
    ...
    shell.setAttribute("aria-label", `Architecture diagram ${index + 1}`);
    shell.innerHTML = `<div class="joint-diagram__toolbar"><span>Interactive system map</span><div><button type="button" data-action="out" aria-label="Zoom out">−</button><button type="button" data-action="fit" aria-label="Fit diagram">Fit</button><button type="button" data-action="in" aria-label="Zoom in">+</button></div></div><div class="joint-diagram__viewport"><div class="joint-diagram__paper"></div></div><figcaption class="joint-diagram__legend"><span style="color:var(--joint-blue)"><i></i>Step</span><span style="color:var(--joint-gold)"><i></i>Decision</span><span style="color:var(--joint-cyan)"><i></i>Evidence</span><span style="color:var(--joint-red)"><i></i>Rejection</span></figcaption>`;
